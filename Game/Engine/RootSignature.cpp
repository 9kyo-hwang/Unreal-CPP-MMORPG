...
FGraphicsRootSignature::~FGraphicsRootSignature() = default;

void FGraphicsRootSignature::Initialize(ComPtr<ID3D12Device> Device, ComPtr<ID3D12GraphicsCommandList> CommandList)
{
	...
	Parameters[0].InitAsConstantBufferView(StaticCast< uint32 >( EConstantBufferViewRegisters::b0 ));	// 전역으로 사용할 b0 레지스터를 위해 RootCBV 추가
...
FComputeRootSignature::FComputeRootSignature() = default;

FComputeRootSignature::~FComputeRootSignature() = default;

void FComputeRootSignature::Initialize(ComPtr<ID3D12Device> Device, ComPtr<ID3D12GraphicsCommandList> CommandList)
{
	CD3DX12_DESCRIPTOR_RANGE Ranges[ ]
	{
